...
extern uint8_t USBD_StringSerial[]; // USB iSerial stored as UTF16 
...
static bool _cmd_sn(const cmd_t *cmd)
{
    ...
    // print iSerial which is hex number in our case
    for (i=2; i<0x1A; i+=2)
    {
        char c = (char)USBD_StringSerial[i];
...
static bool _cmd_pwr_set(const struct _cmd_t *cmd,  const char **pptext)
{
    ...
    if (state)
    {
        HW_VCC_RESET_CLR; // clr reset == power on
        HW_SPI_OE_ENABLE;
    }
    else
    {
        HW_VCC_RESET_SET;  // switch target power off
        HW_SPI_OE_DISABLE; // also disconnect SPI interface
    }
    _pwr_state = state;
    return (true);
}
...
static const cmd_t _CMD_TABLE[] = {
    {"AUTO",      _cmd_auto,    _cmd_auto_set,  "Automatic response reading"},
