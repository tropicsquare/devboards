...
#include "wd.h"
  ...
  #warning "WD disabled" 
  #define WD_ACTIVE 	0
  #define WD_RESET_MODE	0 // zapsat 1, pokud ma wd resetovat, jinak pouze zavola interrupt, ve kterem to zustane viset
#else
  #define WD_ACTIVE 	1
...
#define WD_TIMEOUT		300 // [8ms] jen priblizne, LSI je +-40%
...
#if (WD_ACTIVE != 1)
	// wd neni aktivni, musime ho zapnout
	_wd_start();
#endif // WD_ACTIVE != 1

	IWDG->KR=IWDG_KEY_WRITE_ACCESS_ENABLE;
	IWDG->PR  = 1;  // 
	IWDG->RLR = 1; // co nejrychleji
	while (IWDG->SR != 0)
		;
	IWDG->KR=IWDG_KEY_RELOAD;
	while (1)
